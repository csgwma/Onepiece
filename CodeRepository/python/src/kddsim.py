...
def calc_sims_avg_new(datafile_pre, sim_fun, resultfile):
    ...
    for eachline in reader:
        ...
        vec3 = list()
        if debug:
            for val in tokens[0:skipQix] + tokens[(skipQix+1):10]:
                vec3.append(int(val))
        else:
            for val in tokens:
                vec3.append(int(val))
        ...
        sims = list()
        if debug:
            sims.append(sim_fun(switch[0:skipQix]+switch[(skipQix+1):10], vec3))
            sims.append(sim_fun(entropy[0:skipQix]+entropy[(skipQix+1):10], vec3))
            sims.append(sim_fun(lens[0:skipQix] + lens[(skipQix+1):10], vec3))
            sims.append(sim_fun(score[0:skipQix] + lens[(skipQix+1):10], vec3))
        else:
